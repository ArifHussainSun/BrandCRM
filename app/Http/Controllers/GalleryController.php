...
class GalleryController extends Controller
{
    ...
    public function storeOrUpdate(Request $request)
    {
        ...
        if ($valid) {
            $gallery = Gallery::findOrNew($request->id);
            $isNewGallery = $gallery->id == null;
            ...
            if ($request->hasFile('image')) {
                ...
                $image_destinationPath = $this->galleriesimagepath;
            ...
            $data = array(
                "success" => true,
                "message" => $isNewGallery ? "Gallery Added Successfully" : "Gallery Updated Successfully"
            );
            ...
            if ($isNewGallery) {
                ...
                $gallery->updated_by = $user_id;
            }

            if ($gallery->save()) {
                // Call the touch() method to update the updated_at field
                $gallery->touch();
                $notifyKey = $isNewGallery ? 'added' : 'updated';
                $notify = array(
                    "performed_by" => Auth::user()->id,
                    "title" => $isNewGallery ? "Added New Gallery" : "Updated Gallery",
                    "desc" => array(
                        $notifyKey . '_title' => $request->name,
                        $notifyKey . '_description' => $request->desc,
                    )
                );
                ...
                $data["success"] = false;
                $data["message"] = $isNewGallery ? "Gallery Not Added Successfully." : "Gallery Not Updated Successfully.";
                Session::flash('error', $data["message"]);
            }

            return redirect()->route('gallery.list')->with($data);
        } else {
    ...
    public function delete(Request $request)
    {
        ...
        return response()->json([
            "success" => $success,
            "message" => $success ? "Gallery Deleted Successfully!" : "Failed to Delete Gallery!"
        ]);
    }
    ...
    {
        $Gallery = Gallery::onlyTrashed()->find($request->id);
        $success = $Gallery->forceDelete();
        $response = [
            "success" => $success,
            "message" => $Gallery->trashed() ? "Gallery Destroy Successfully!" : "Failed to Destroy Gallery!"
        ];
    ...
    {
        $gallery = Gallery::onlyTrashed()->findOrFail($request->id);
        $success = $gallery->restore();

        $response = [
            "success" => $success,
            "message" => $success ? "Gallery Restored Successfully!" : "Failed to Restore Gallery!",

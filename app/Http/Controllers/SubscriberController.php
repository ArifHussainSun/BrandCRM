...
class SubscriberController extends Controller
{
    ...
    public function storeOrUpdate(Request $request)
    {
        ...
        if ($valid) {
            ...
            $isNewSubscriber = $subscriber->id == null;

            $subscriber->name = $request->input('name'); 	
            $subscriber->phone = $request->input('phone');
            $subscriber->email = $request->input('email');
            $subscriber->data = json_encode($request->input('data'));

            $management = User::role(['Admin', 'Brand Manager'])->get();
            $management->pluck('id');
            $data = array(
                "success" => true,
                "message" => $isNewSubscriber ? "Subscriber Added Successfully" : "Subscriber Updated Successfully"
            );

            // Set the created_by and updated_by values
            $user_id = Auth::user()->id;
            if ($isNewSubscriber) {
                $subscriber->created_by = $user_id;
            } else {
                $subscriber->updated_by = $user_id;
            }

            if ($subscriber->save()) {
                // Call the touch() method to update the updated_at field
                $subscriber->touch();
                $notifyKey = $isNewSubscriber ? 'added' : 'updated';
                $notify = array(
                    "performed_by" => Auth::user()->id,
                    "title" => $isNewSubscriber ? "Added New Subscriber" : "Updated Subscriber",
                    "desc" => array(
                        $notifyKey . '_title' => $request->input('name'),
                        $notifyKey . '_description' => $request->email,
                    )
                );
                ...
                $data["success"] = false;
                $data["message"] = $isNewSubscriber ? "Subscriber Not Added Successfully." : "Subscriber Not Updated Successfully.";
    ...
    public function delete(Request $request)
    {
        ...
        return response()->json([
            "success" => $success,
            "message" => $success ? "Subscriber Deleted Successfully!" : "Failed to Delete Subscriber!"
        ]);
    }
    ...
    public function restore(Request $request, $id)
    {
        ...
        $success = $subscriber->restore();

        $response = [
            "success" => $success,
            "message" => $success ? "Subscriber Restored Successfully!" : "Failed to Restore Subscriber!"
        ];
    ...
    public function destroy(Request $request)
    {
        ...
        $success = $subscriber->forceDelete();
        $response = [
            "success" => $success,
            "message" => $subscriber->trashed() ? "Subscriber Destroy Successfully!" : "Failed to Destroy Subscriber!"
        ];

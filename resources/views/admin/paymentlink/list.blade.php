                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        <label for="customer_id">Customer (IF EXISTING)</label>
                                    ...
                                    <div class="form-group">
                                        <label for="original_price">Discount Type</label>
                                        <select class="form-control input-sm select2" name="discount_type" id="discount_type" style="width: 100%;">
                                            <option value="" selected disabled>Select Discount Type</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="item_descriptionription">Item Descripion</label>
                                        <input type="text" name="item_description" id="item_description" class="form-control input-sm">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label for="converted_amount">Payment Gateway</label>
                                        <select class="form-control input-sm select2" name="payment_gateway"  id="payment_gateway" style="width: 100%;">
                                            <option value="" selected disabled>Payment Gateway</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="coupon_id">Payment Type</label>
                                    ...
                                    <div class="form-group">
                                            <strong><label for="short_address" class="col-sm-12 col-form-label">Sale Type</label></strong>
                                                <label>
                                                    <input type="radio" class="form-check-input" name="sale_type" value="" >
                                                </label>  Fresh Sales
                                                <label>
                                                    <input type="radio" class="form-check-input" name="sale_type" value="" >
                                                </label>      Upsell
 ...
 <script>
    ...
    $(document).ready(function() {
        ...
        var table = $('#paymentlink').DataTable({
                ajax: route('payment-link-generator.list'),
                serverSide: true,
                processing: true,
                scrollX: true,
                aaSorting:[[0,"desc"]],
        ...
        $(document).on('click','.btn-view',function(){
            $.ajax({
                url: route('payment-link-generator.detail.view'),
                type: "post",
                data: {id: $(this).data('id')},
                success: function (data) {
                    console.log(data);
                    var customer_name = ((data.customer!=null) ? (data.customer.first_name +" "+ data.customer.last_name) :'');
                    $('#id').html(data.id);
                    $('#customer_id').html(customer_name);
        ...
        $(document).on('click','.btn-delete',function(){
            ...
            Swal.fire({
                ...
                if (t.value) {
                    ...
                    if(!id) return;
                    $.ajax({
                        url: route('payment-link-generator.remove'),
                        type: "POST",
                        data:{id:id},
                        dataType: 'JSON',
    ...
    $(document).on('click','.btn-copy',function(){
        ...
        var id = $(this).data('id');
        /* Get the text field */
        var copyText = document.getElementById('copy_link_'+id);
        ...
        unsecuredCopyToClipboard(copyText.value);
         ...
         showMsg(type="success", message="Link Copied Successfully!");
    });  

    $(document).on('click','.btn-extend',function(){
            var id = $(this).data('id');
            $.ajax({
                url: route('payment-link-generator.extend-validity'),
                type: "post",
                data: {id: id},
                ...
                success: function (data) {
                   if(data.success==true){
                    let table = $('#paymentlink').DataTable();
                    table.row('#' + id).remove().draw(false)
                    showMsg(type=data.type, message=data.message);
                   }
                   else
                   {
                    showMsg(type=data.type, message=data.message);
